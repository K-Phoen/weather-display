...
#include "renderer.h"
#include "prometheus/TimeSeries.h"
#include "prometheus/PromClient.h"
#include "prometheus/WriteRequest.h"
...
static combined_data_t combined_data;
...
TimeSeries tempTs(5, "homelab_sensors_temp", "{room=\"living_room\"}");

// Humidity in %
TimeSeries humidityTs(5, "homelab_sensors_humidity", "{room=\"living_room\"}");

// Atmospheric pressure in Pascal.
TimeSeries pressureTs(5, "homelab_sensors_pressure", "{room=\"living_room\"}");
...
void setup()
{
...
#endif
  int rxStatus = getCombinedData(client, combined_data);
  if (rxStatus != HTTP_CODE_OK)
  {
    killWiFi();
    statusStr = "Backend call";
  ...
  if(bme.begin(BME_ADDRESS, &I2C_bme))
  {
    ...
    inPressure = bme.readPressure();    // in Pascal
  ...
  // Send some prometheus metrics.
  PromClient promClient(client);
  promClient.setUrl(PROM_PUSH_HOST.c_str());
  promClient.setPath(PROM_PUSH_URL.c_str());
  promClient.setPort(PROM_PUSH_PORT);
  promClient.setUser(PROM_PUSH_USER.c_str());
  promClient.setPass(PROM_PUSH_PASS.c_str());
  #if DEBUG_LEVEL >= 1
  ...
  req.addTimeSeries(tempTs);
  req.addTimeSeries(humidityTs);
  req.addTimeSeries(pressureTs);
  #if DEBUG_LEVEL >= 1
  req.setDebug(Serial);
  #endif
  ...
  tempTs.addSample(time, inTemp);
  pressureTs.addSample(time, inPressure);
  humidityTs.addSample(time, inHumidity);
  ...
  tempTs.resetSamples();
  pressureTs.resetSamples();
  humidityTs.resetSamples();
  ...
  do
  {
    drawCurrentConditions(combined_data.weather_data.current, combined_data.weather_data.daily[0],
                          combined_data.air_pollution, inTemp, inHumidity);
    drawForecast(combined_data.weather_data.daily, timeInfo);
    drawLocationDate(CITY_STRING, dateStr);
    drawCalendarEvents("Calendar", combined_data.events);
#if DISPLAY_ALERTS
    drawAlerts(combined_data.weather_data.alerts, CITY_STRING, dateStr);
#endif
