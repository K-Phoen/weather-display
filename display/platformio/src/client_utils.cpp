...
/* Perform an HTTP GET request get a bunch of data.
 * If data is received, it will be parsed and stored in `r` parameter.
 *
 * Returns the HTTP Status Code.
 */
#ifdef USE_HTTP
  int getCombinedData(WiFiClient &client, combined_data_t &r)
#else
  int getCombinedData(WiFiClientSecure &client, combined_data_t &r)
#endif
{
  int attempts = 0;
  ...
  String uri = "/api/combined";

  Serial.print(TXT_ATTEMPTING_HTTP_REQ);
  Serial.println(": " + API_HOST + uri);
  int httpResponse = 0;
  while (!rxSuccess && attempts < 3)
  {
    ...
    http.setAuthorizationType("");
    http.setAuthorization(SHARED_SECRET.c_str());
    http.begin(client, API_HOST, API_PORT, uri);
    httpResponse = http.GET();
    if (httpResponse == HTTP_CODE_OK)
    {
      jsonErr = deserializeCombinedData(http.getStream(), r);
      if (jsonErr)
      {
