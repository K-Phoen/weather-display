...
#include "extra_icons/icons_48x48.h"
...
/* This function is responsible for drawing the SL departures list.
 */
void drawCalendarEvents(const String &title, std::vector<calendar_event_t> const &events)
{
  const int xPos = 320;
  const int yPosTitle = 235; // to roughly match the "Sunrise" text block

  display.drawInvertedBitmap(xPos, yPosTitle - (48 / 2 + 8),
                             calendar_48x48, 48, 48, GxEPD_BLACK);

  display.setFont(&FONT_16pt8b);
  drawString(xPos + 48 + 10, yPosTitle, title, LEFT, ACCENT_COLOR);

  const int yPosEvent = yPosTitle + 20; // bit of spacing
  ...
  {
    const calendar_event_t &event = events.at(i);
    char buffer[256];

    strftime(buffer, sizeof(buffer), "%b %a %d %H:%M", &event.start);

    drawString(
      xPos, yPosEvent + (30 * (i+1)),
      String(buffer)+" - "+event.summary,
      LEFT
    );

    // cap the maximum number of events being displayed.
    if (i == 5) {
      break;
    }
  }
