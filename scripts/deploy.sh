...
VALUES_FILE="k8s/weather-display/values.yaml"
...
debug "Updating ${VALUES_FILE}"
yq -i ".weather_display.tag = \"${IMAGE_TAG}\"" "${k8s_repo_workspace}/${VALUES_FILE}"

debug "Adding changes to git staging area"
git_run "${k8s_repo_workspace}" add .

debug "Committing change"
git_run "${k8s_repo_workspace}" commit -m "Automated weather-display release"
